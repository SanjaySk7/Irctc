...
const Users = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.get(`http://localhost:8081/get-trains/${formData.fromStation}/${formData.toStation}`);
  ...
  const handleBookTrain = async (trainId) => {
    try {
      ...
      const response = await axios.post(`http://localhost:8081/book-train`, { trainId, userId });
